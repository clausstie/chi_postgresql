 ...
 <script language="JavaScript" src="../script/inventoryScript.js"></script>
  <SCRIPT type="text/javascript">
    function validateForm(form) 
	{
	 ...
	 else if (trim(form.batch_id.value) != "")
	 {
	 	if(isNumber(form.batch_id.value)==false)
    	{
	        alert("Please fill a valid batch id");
    	    form.batch_id.focus();
       		return false;
    	}
	 }
...
{%>
<body onload="document.batch.batch_id.focus();">
<%
}
else
{%>
...
<br>
<center>
	<form method="post" action="<%= Attributes.HISTORY_BASE %>?action=batch_history&code1=yes" name="batch" onSubmit="return validateForm(this)">
	    <table class="box" cellpadding="1" cellspacing="2" border="0" width="350">
  		<TR><TH colspan="4" class="blue">Search</TH></TR>
	        <tr>
	            ...
	            <td><input type="text" name="batch_id" tabindex="1" class="w200"></td>
	        </tr>
	    </table><br>
...
//display error message if invalid batch id.
if(request.getParameter("errorCode1") != null)
{%>
	<h3>Not a valid batch id entered, please try again.</h3>
<%
}
...
{%>
	<h3>Batch id is not registered in the system.</h3>
<%
}
...
if (request.getParameter("code1") != null)
{
	String id = history.getBatch_id();
	boolean check = history.getBatchHistory(id);
...
<CENTER>
	<table class="box" cellpadding="1" cellspacing="1" width="99%">
		<tr>
			<th colspan="2" align="left">Batch id</th>
		...
		<%
			...
			else
			{
				...
				<tr>
					<td colspan="8">
						<i>No history lines for the batch</i>
					</td>
				</tr>
				<%
			}%>
...
<%}//check was ok
  else//check not ok
  {
   ...
   if(status == 1)
   {
   	response.sendRedirect(Attributes.HISTORY_BASE+"?action=batch_history&errorCode1=yes");
   }
   else if(status == 2)
   {
   	response.sendRedirect(Attributes.HISTORY_BASE+"?action=batch_history&errorCode2=yes");
   	}
   else
   {  
  %>
  <h2>Batch History - Error</h2>
	<img src="<%=Attributes.IMAGE_FOLDER%>/Divider1a800.png">
	<br>
	<CENTER>
	<h3>Error in display of history for the batch, please try again.</h3>
	</CENTER> 
	<%
	}
  }
