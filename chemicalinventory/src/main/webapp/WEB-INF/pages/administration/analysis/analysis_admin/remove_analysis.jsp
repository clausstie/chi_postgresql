...
if (request.getParameter("code2") != null)
{
	...
	boolean check = analysis.removeAnalysis(id, user);

	if(check)
	{%>
		<center>
			<h3>The analysis has been removed!</h3>
		</center>
	<%	
	}
	else
	{%>
		<center>
			<h3>ERROR the analysis could not be removed!</h3>
		</center>
	<%
	}
...
<br>
<p>Select the analysis to remove</p>
<center>
	<table class="box" cellspacing="0" cellpadding="1" width="750">
		...
		<tbody>
			<tr>
				<td colspan="6">
					<iframe id="list_frame" frameborder="0" scrolling="yes" src="<%= Attributes.ANALYSIS_ADMINISTRATOR_BASE %>?action=analysis_list_remove" width="750" height="500" marginwidth="0" name="list_frame">
					</iframe>
				</td>
			</tr>				
		</tbody>
	...
	<br>
	  <center>
		...
		<form method="post" action="">
			<input class="submit" type="submit" name="Remove" value="Remove" onclick="this.form.action='<%= Attributes.ANALYSIS_ADMINISTRATOR_BASE %>?action=remove_analysis&code2=yes'">
			<input class="submit" type="submit" name="Cancel" value="Cancel" onclick="this.form.action='<%= Attributes.ANALYSIS_ADMINISTRATOR_BASE %>?action=remove_analysis'">
