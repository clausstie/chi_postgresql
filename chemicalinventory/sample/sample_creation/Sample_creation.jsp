...
<jsp:useBean id="container_list" class="chemicalinventory.container.ContainerList" scope="page"/>
...
function validateForm(form) 
{
 ...
 {
  alert("Please select a compound for this sample");
  form.compound.focus();
  return false;
 }
 ...
 {
  alert("A container must be selected for the creation of sample.");
...
if(com_name != null && com_id != null)
{
	...
	//create a list of batches available to attach to this sample
	list = sample.batchsOnCompoundList(com_id);
	
	//Create a list of containers for the compound, that the user has access to.
	container_list.getContainerList(user, Util.getIntValue(com_id));
	c_list = container_list.getList();
}
else
{
...
<CENTER>
<form method="post" action="<%= Attributes.SAMPLE_BASE %>?action=create_sample&code1=yes" onSubmit="return validateForm(this)" name="sample">
   <table class="box" cellpadding="1" cellspacing="2" border="0" width="660">
 		<TR><TH colspan="4" class="blue">Sample Creation</TH></TR>
 		<tr>
			<th align="left">Compound</th>
			<td><input size="79" type="text" name="compound" value="<%=com_name%>" <%=readonly%>>
				<input type="hidden" name="compound_id" value="<%=com_id%>">
			</td>
			<td>
				<a href="#" onclick="openWindow('<%=Attributes.JSP_BASE%>?action=s_search&return_to=create_sample', 1)"><img src="<%=Attributes.IMAGE_FOLDER%>/plus_mark.png" height="15" width="15" border="0"></a>
			</td>
		</tr>
		<tr>
			...
			<td colspan="2">
				...
				<select name="container_id" style="width: 280px">
		...
		<tr>
			...
			%>
				<select name="map_id" style="width: 280px">
					<option value="" selected="selected">[--- SELECT ---]</option>
				<%
	...
	<script language="javascript">toggle(getObject('exp1098695543_link'), 'exp1098695543');</script>
		...
		<%
			...
			{%>
			<select name="batch" style="width: 250px" size="1">
					<option value="0">[-- SELECT --]</option>
			<%
				for(int i = 0; i < list.size(); i++)
				{
					String batch_id = (String) list.get(i);
					%>
		...
		<tr>
			...
			<td colspan="2"><textarea rows="5" cols="65" name="remark"></textarea>
			</td>
		</tr>
	</table><br>
	<input class="submit" type="submit" name="Submit" value="Submit" onclick="this.form.action='<%=Attributes.SAMPLE_BASE%>?action=create_sample&code1=yes'">
...
{%>
	<br>
	<center>
		<H3>The registration failed, select a compound for the sample!!</H3>
	<center><%
}
...
{
	sample.setUser(user);
	boolean check =	sample.registerSample();
	String id1 = sample.getSample_id();

	if(check)
	{	
		//send the user on...
		response.sendRedirect(Attributes.SAMPLE_BASE+"?action=create_sample&code2=yes&sample_id="+id1);
	}
	else
	{
		int code = sample.getErrorCode();
		
		if(code == 2)//no compound selected
				response.sendRedirect(Attributes.SAMPLE_BASE+"?action=create_sample&error1=yes");
		else if(code == 4)
				response.sendRedirect(Attributes.SAMPLE_BASE+"?action=create_sample&error4=yes");
		else//compound selected but another error happend
				response.sendRedirect(Attributes.SAMPLE_BASE+"?action=create_sample&error2=yes");
	}
}//end code 1
			...
			<tr>
				<td>
					<table border="0" width="100%">
						...
						<tr>
							<th align="left" width="160">Container_id:</th>
