...
-->
  ...
  <span class="textboxadm">
   ...
   <center>
   	<form method="post" action="<%=Attributes.BATCH_BASE%>?action=unlock_batch&code1=yes" name="resource_form" onsubmit="return validateForm(this)">
   ...
   <%
    ...
    if (request.getParameter("code1") != null)
 	...
 	%>
			...
			<img src="<%=Attributes.IMAGE_FOLDER%>/Divider1a800.png">
		 	  <center>
				<form method="post" action="<%=Attributes.BATCH_BASE%>?action=unlock_batch&code2=yes">
				    ...
				    {
				    %>
				    	<p>This batch is not locked, and you here have the ability to lock the batch</p>
				    	<br/>
			   	    ...
			   	    <input class="submit" type="submit" value="Cancel" onclick="this.form.action='<%= Attributes.BATCH_BASE %>?action=unlock_batch'">
				</form>
			</center>
	<%
		}
		...
		{%>
		 <h2>Un-Lock Batch - Error Status</h2>
		<img src="<%=Attributes.IMAGE_FOLDER%>/Divider1a800.png">
		<br/>
	...
	if (request.getParameter("code2") != null)
 	{
 		batch_id = batch.getBatch_id();
 		batch.setUser(user);
 		boolean lock = batch.unlockBatch(batch_id);
 		
 		if(lock)
 		{
 			response.sendRedirect(Attributes.BATCH_BASE+"?action=unlock_batch&code3=yes&batch_id="+batch_id);
 		}
 		else
 		{
 			response.sendRedirect(Attributes.BATCH_BASE+"?action=unlock_batch&errorcode2=yes&batch_id="+batch_id);
 		}
 	}
 	...
 	if (request.getParameter("code3") != null)
 	{
 		...
 		%>
 		 <h2>Un-Lock Batch - Status</h2>
		<img src="<%=Attributes.IMAGE_FOLDER%>/Divider1a800.png">
		<br/>
		<h3>The batch number <%=batch_id%> has been un-locked.</h3>
 		<%
 	...
 	if (request.getParameter("errorcode2") != null)
 	{
		...
		<img src="<%=Attributes.IMAGE_FOLDER%>/Divider1a800.png">
		<br/>
		<h3>The batch number <%=batch_id%> could not be un-locked.</h3>
 		<%
 	}
