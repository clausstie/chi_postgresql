...
<title>Unlock the sample, but keep the results locked</title>
</head>
<%
...
if(request.getParameter("code1") == null && request.getParameter("code2") == null)
{%>
<h2>Select sample to un-lock</h2>
<img src="<%=Attributes.IMAGE_FOLDER%>/Divider1a800.png">
<br>
<center>
<form method="post" action="<%= Attributes.SAMPLEAPPROVER_BASE %>?action=unlock_sample&code1=yes" name="sample" onsubmit="return validateForm(this)">
   <table class="box" cellpadding="1" cellspacing="2" border="0" width="370">
	...
	<input class="submit" type="submit" name="Submit" value="Submit" onclick="this.form.action='<%=Attributes.SAMPLEAPPROVER_BASE%>?action=unlock_sample&code1=yes'">&nbsp;
...
if(request.getParameter("code3") != null)//receipt for succes on locking a sample
{
	...
	%><h3>The sample (id: <%=sample_id%>) has been un-locked!</h3><%
}
...
%>
<h2>Display sample information - Un-lock sample</h2>
<img src="<%=Attributes.IMAGE_FOLDER%>/Divider1a800.png">
<br>	
<center>
<%if(request.getParameter("errorcode1")!=null)
{
	%><h3>An error orcurred, sample could not be un-locked, please try again.</h3><%
}%>
<%//if the sample is not locked display a message..
isLocked = sample.getLocked().trim();
if(isLocked.equals("F"))
{
	%><h3>Sample is NOT locked!!!</h3><%
}%>
<form method="post" action="<%= Attributes.SAMPLEAPPROVER_BASE %>?action=unlock_sample&code2=yes" name="sample">
	...
	<%//only display buttons if the sample is not priviously locked
	if(isLocked.equals("T"))
	{
	  %><input class="submit" type="submit" name="Submit" value="Un-lock Sample" onclick="this.form.action='<%=Attributes.SAMPLEAPPROVER_BASE%>?action=unlock_sample&code2=yes&sample_id=<%=sample_id%>'">
		<input class="submit" type="submit" name="Cancel" value="Cancel" onclick="this.form.action='<%=Attributes.SAMPLEAPPROVER_BASE%>?action=unlock_sample'"><%
	}%>	
</form><br>
</center>
  <%}
...
{
	sample_id = sample.getSample_id();
	boolean checkLock = sample.unLockSample(sample_id, user);
	
	if(checkLock)
	{
		response.sendRedirect(Attributes.SAMPLEAPPROVER_BASE+"?action=unlock_sample&code3=yes&sample_id="+sample_id);
	}
	else//error sample could not be locked..
	{
		response.sendRedirect(Attributes.SAMPLEAPPROVER_BASE+"?action=unlock_sample&code1=yes&errorcode1=yes&sample_id="+sample_id);
	}
}
%>
</span>
